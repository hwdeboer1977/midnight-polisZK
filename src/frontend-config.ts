...
if (pin) {
  fs.writeFileSync(
    envLocal,
    `# Generated by \`npm run frontend:config\` from PAYROLL_CONTRACT. Do not edit.\n` +
      `VITE_PAYROLL_CONTRACTS=${pin}\n`
  );
  console.log(`Pinned payroll contracts -> ${pin}`);
} else if (fs.existsSync(envLocal)) {
  // Removed when the pin is cleared, so unsetting the variable actually unpins
  // rather than leaving a stale file that keeps filtering.
  fs.unlinkSync(envLocal);
  console.log("Cleared the payroll contract pin");
}
process.exit(0);
